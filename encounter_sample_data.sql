...
SELECT
    -- Encounter IDs with intentional duplicates (late updates)
    CONCAT('E', LPAD((g.id % 850 + 1)::TEXT, 4, '0')) AS encounter_id,

    -- Patients P001–P050 (skew creates high-volume patients)
    CONCAT('P', LPAD((g.id % 50 + 1)::TEXT, 3, '0')) AS patient_id,

    -- Facilities F001–F005
    CONCAT('F', LPAD((g.id % 5 + 1)::TEXT, 3, '0')) AS facility_id,

    -- Encounter dates across Jan–Mar 2025
